...
namespace cmind{
    ...
    LinearRegression::LinearRegression(): Algorithm(Algorithms::LinearRegression){
    ...
    void LinearRegression::compile(Loss& loss_func, Optimizer& opt){
        Losses loss_type = loss_func.get_loss_type();
        if(loss_type != Losses::MSE && loss_type != Losses::MAE && loss_type != Losses::HuberLoss && loss_type != Losses::RidgeLoss && loss_type != Losses::LassoLoss && loss_type != Losses::ElasticNetLoss){
            std::cout << "Compile: Loss function not supported" << std::endl;
            std::cout << "Linear Regression supported loss functions: MSE, MAE, RidgeLoss, LassoLoss, ElasticNetLoss" << std::endl;
            abort();
        }
    ...
    void LinearRegression::train(Dataset& ds, const size_t epochs){
        Losses loss_type = this->loss_func_->get_loss_type();

        if(this->weights_ == nullptr){
            this->weights_ = new Tensor<float>({ds.shape()[1], 1});
            this->weights_->randomize();
        }
        else if(this->weights_->shape()[0] != ds.shape()[1]){
            std::cout << "Train: Input shape does not match" << std::endl;
            abort();
        }
        if(loss_type == Losses::RidgeLoss)
            ((RidgeLoss*)this->loss_func_)->set_weights(this->weights_, this->bias_);
        else if(loss_type == Losses::LassoLoss)
            ((LassoLoss*)this->loss_func_)->set_weights(this->weights_, this->bias_);
        else if(loss_type == Losses::ElasticNetLoss)
    ...
    // BinaryLogisticRegression constructor
    BinaryLogisticRegression::BinaryLogisticRegression(): Algorithm(Algorithms::BinaryLogisticRegression){
        this->bias_ = new Tensor<float>({1});
        this->bias_->randomize(); 
    }

    // Load model from file
    BinaryLogisticRegression::BinaryLogisticRegression(const std::string& filepath): Algorithm(Algorithms::BinaryLogisticRegression){
        std::ifstream file(filepath);
    ...
    // Returns the bias
    const Tensor<float>* BinaryLogisticRegression::bias()const{
        return this->bias_;
    }

    // Runs the algorithm and returns the result
    const Tensor<float> BinaryLogisticRegression::operator()(const Tensor<float>& input)const{
        // Tensor<float> output({input.shape()[0]});
        // output.fill(0.0);
        ...
        output += mat_mul(input, *this->weights_);
        output = 1.0f/(exp(-output)+1);
        return output.flatten();
    }

    // Sets the loss function and the optimizer
    void BinaryLogisticRegression::compile(Loss& loss_func, Optimizer& opt){
        Losses loss_type = loss_func.get_loss_type();
        if(loss_type != Losses::BCE){
            std::cout << "Compile: Loss function not supported" << std::endl;
            std::cout << "Logistic Regression supported loss functions: BCE" << std::endl;
            abort();
        }
    ...
    // Trains the algorithm
    void BinaryLogisticRegression::train(Dataset& ds, const size_t epochs){
        if(this->weights_ == nullptr){
            this->weights_ = new Tensor<float>({ds.shape()[1], 1});
            this->weights_->randomize();
        }
    ...
    // Predicts for a single input
    const Tensor<float> BinaryLogisticRegression::predict(const std::vector<float> input)const{
        if(input.size() != this->weights_->shape()[0]){
            std::cout << "Predict: Input size does not match" << std::endl;
            abort();
        }
        ...
        return 1.0f/(exp(-output)+1);
    }

     // Saves the weights
    void BinaryLogisticRegression::save(const std::string& filepath)const{
        std::ofstream file(filepath);
        file << std::to_string((int)this->alg_type) + "\n";
    ...
    // Loads the weights
    void BinaryLogisticRegression::load(const std::string& filepath){
        std::ifstream file(filepath);
        std::string line;
